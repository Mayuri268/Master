...
def agentdetails_home():
    try:
        return render_template("AgentList.html")
    except Exception as ex:
        logfile = open('log.txt', 'w')
...
def UploadExcel():
    try:
            ...
            for i in range(0,len(df)):
                ...
                agent_data["onboarding_date"]= datetime.datetime.strptime(agent_data["onboarding_date"], '%d/%m/%Y').date()
                agent_data["expiration_date"]= datetime.datetime.strptime(agent_data["expiration_date"], '%d/%m/%Y').date()
                agent_data["onboarding_date"]=datetime.datetime.combine(agent_data["onboarding_date"],datetime.datetime.min.time())
                agent_data["expiration_date"]=datetime.datetime.combine(agent_data["expiration_date"],datetime.datetime.min.time())
                if db.master.find_one({"emp_id":agent_data["emp_id"],"role":"agent"},{"_id":0}):
                    break
                else:
                    db.master.insert_one(agent_data) #change the database name : IMPORTANT 
