...
def logincheck():
    try:
        ...
        def user(user_id):
            ...
            user_data=list(db["master"].find({"user_id":user_id},{"role":1}))
            print(user_data)
            ...
            if user_data[0]["role"]=="supervisor":
                ...
                x=list(db.master.find({"project_name":user_project,"role":"agent"},{"user_id":1,"_id":0}))
                agent_names=[]
                for i in range(0,len(x)):
                    agent_names.append(x[i]["user_id"])
        ...
        if user_details:
            ...
            # return render_template("login.html",data="Invalid user_id or password")
            # return (json.dumps(["Invalid user_id or password"])) 
            data={"Status":"Error","Msg":"Invalid username or password"}
            logging.error("Invalid username or password")
            return json.dumps(data) 
    except Exception as ex:
    ...
    try:               
        ...
        if session['logged_in_user_role']=="supervisor":
            ...
            for i in all_details:
                ...
                if i["user_id"] in session["agent_names"]:
                    data.append(i)
            all_details=data
...
def OnboardedAgent(PageNo):
    try:
        ...
        all_details = list(db["master"].find({},{"_id": 0, "user_id": 1, "user_name": 1, "onboarding_date": 1,
                                                "expiration_date": 1, "facial_img": 1, "status": 1}).skip(offset).limit(limit)) 
        all_details2 = list(db["master"].find({},{"_id": 0, "user_id": 1, "user_name": 1, "onboarding_date": 1,
...
def FilterOnboardedAgent():
    try:
        ...
        if  status != "" and name != "": 
            query_result=list(db["master"].find({"status":status,"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}).skip(offset).limit(limit))

            query_result2=list(db["master"].find({"status":status,"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}))

        elif name != "" and  status== "":
            query_result=list(db["master"].find({"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}).skip(offset).limit(limit))

            query_result2=list(db["master"].find({"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}))

        elif status != "" and name =="":
            query_result=list(db["master"].find({"status":status},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}).skip(offset).limit(limit))

            query_result2=list(db["master"].find({"status":status},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}))
    
        else:
            query_result=list(db["master"].find({},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}).skip(offset).limit(limit))

            query_result2=list(db["master"].find({},{"_id": 0, "user_id": 1, "user_name": 1,
            "onboarding_date": 1, "expiration_date": 1, "facial_img": 1, "status": 1}))
...
def agentdetails_home():
    try:
        ...
        db = cluster["Proctoring_Josh"]                              
        project_names = list(db["dailySession"].distinct("project_name"))
        if session['logged_in_user_role']=="supervisor":
...
def violation_details():
    try:
        ...
        query_result = list(db.violation.find({"marked_as":"TBM"}, {"_id": 1, "violation_type": 1,"user_id":1,"marked_as": 1,"created_date":1,
...
def violation_details_filter(project,fro,to):
    try:
        ...
        time_from=datetime.datetime.combine(time_from,datetime.datetime.min.time())
        time_to=datetime.datetime.combine(time_to,datetime.datetime.min.time())
        data=list(db.violation.find({"project_name":project,"created_date": {"$gte":time_from,"$lte":time_to}}))
        for i in data:
                i["_id"]= str(i["_id"])
...
def update_markedas(id,marked_as,user_id):
    try:
        dbResponse = db.violation.update_one({"_id": ObjectId(id)},{"$set": {"marked_as":marked_as}})
        x=list(db.violation.aggregate(
                        ...
                        "COUNT": {"$sum": 1}}}]))
        count=x[0]["COUNT"]
        db.master.update_one({"user_id":user_id},{"$set": {"flags":count}})
...
def violation_update():
    try:
        ...
        project_names = list(db["master"].distinct("project_name"))
        dbResponse = db.master.update_many({"project_name":request.form["projectName"]},
...
def configurations_list(project_name):
    try:
        data=list(db.master.find({"project_name":project_name},{"violation_filter":1,"_id":0}))
        return json.dumps(data[0])
    except Exception as ex:
        logfile = open('log.txt', 'w')
...
def userManagementData(PageNo):
    try:
        ...
        data1=list(db.dailySession.find({"session_date_string":str(today)},{"_id":0,"user_id":1,"user_name":1,"project_name":1,"login_time":1,"logout_time":1,"session_status":1}))
...
def FilterbyAgents():
    try:
        ...
        today = datetime.datetime.combine(date.today(), datetime.datetime.min.time())
        if  project != "" and name != "": 
            data=list(db.dailySession.find({"session_date_string":str(today),"project_name":project,"user_name":{"$regex":name}},{"_id": 0,"user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}).skip(offset).limit(limit))
            data2=list(db.dailySession.find({"session_date_string":str(today),"project_name":project,"user_name":{"$regex":name}},{"_id": 0,"user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}))
        elif name != "" and  project== "":
            data=list(db.dailySession.find({"session_date_string":str(today),"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}).skip(offset).limit(limit))
            data2=list(db.dailySession.find({"session_date_string":str(today),"user_name":{"$regex":name}},{"_id": 0, "user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}))
            ...
            data2=list(db.dailySession.find({"session_date_string":str(today),"project_name":project},{"_id": 0, "user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1, "session_status": 1})) 
        else:
            data=list(db.dailySession.find({"session_date_string":str(today)},{"_id": 0, "user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}).skip(offset).limit(limit))
            data2=list(db.dailySession.find({"session_date_string":str(today)},{"_id": 0, "user_id": 1, "user_name": 1,"project_name":1,"login_time":1,"logout_time":1,"session_status": 1}))
...
def FilterbyViolation():
    try:
        ...
        if  violation != "" and name != "": 
            query_result = list(db.violation.find({"marked_as":"TBM","user_id":{"$regex":name},"violation_type":violation},{"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}).skip(offset).limit(limit))
            query_result2 = list(db.violation.find({"marked_as":"TBM","user_id":{"$regex":name},"violation_type":violation},{"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}))
        elif name != "" and  violation== "":
            query_result = list(db.violation.find({"marked_as":"TBM","user_id":{"$regex":name}}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}).skip(offset).limit(limit))
            query_result2 = list(db.violation.find({"marked_as":"TBM","user_id":{"$regex":name}}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}))
            ...
            query_result = list(db.violation.find({"marked_as":"TBM","violation_type":violation}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}).skip(offset).limit(limit))
            query_result2 = list(db.violation.find({"marked_as":"TBM","violation_type":violation}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}))
        else:
            query_result = list(db.violation.find({"marked_as":"TBM"}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}).skip(offset).limit(limit))
            query_result2 = list(db.violation.find({"marked_as":"TBM"}, {"_id": 1, "violation_type": 1,"user_id":1, "user_name": 1, "marked_as": 1,
                                                    "reviewed_by": 1, "violation_image": 1,"project_name":1,"created_date":1}))
        for i in query_result:
            i["_id"] = str(i["_id"])
            i["created_date"]=str(i["created_date"].date())
